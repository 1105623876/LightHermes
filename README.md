...
- ✅ **记忆系统检索修复**（中英文混合分词，索引搜索恢复正常，7/7 测试通过）
...
**最近更新** (2026-05-02):
- ✅ **记忆系统检索修复**
  - 问题：`split()` 无法处理中文分词，导致检索完全失效
  - 解决：实现轻量级中英文混合分词（中文按字，英文按词）
  - 测试：7/7 系统测试全部通过，记忆召回功能恢复正常
- ✅ **固定记忆文件格式修复**
  - 修复 SOUL.md 和 USER.md 的 frontmatter 格式
  - 添加标准的元数据头，确保文件可以被正确解析
- ✅ **pytest 测试框架**
  - 添加完整的单元测试套件（记忆系统、Adapter）
  - 添加性能基准测试（索引、搜索、召回、可扩展性）
  - 配置 pytest 框架和共享 fixtures
