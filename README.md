...
**当前状态（2026-05-12）**: v0.3.3 已发布；Phase 3.1 轻量工具与 Channel 插件系统实现完成，等待合并评审。
...
**测试覆盖**:
- pytest 测试：140/140 通过 (100%)
- 覆盖模块：记忆系统、Adapter、MiniMax 流式响应、自进化质量评估、失败报告、反模式提示、记忆生命周期、记忆蒸馏、上下文压缩、CLI、性能基准、工具/Channel 插件系统
...
- 新增插件系统测试：工具参数校验、本地工具插件、ChannelRegistry、Channel 插件、Core 插件集成和非 strict 错误隔离
...
**下一步计划**:
- **优先**: 评审并合并 Phase 3.1 插件系统分支，作为后续 v0.4.x / v0.5.0 候选基础
- **中优先**: 基于本地插件边界补充示例插件、开发指南和按需工具生态集成
- **中低优先**: 记忆系统性能优化（缓存、批量操作、索引优化评估）
- **低优先**: `self_state` 只读自省工具、多模态输入、VS Code/Web UI/Docker 等生态扩展

**最近更新** (2026-05-12):
- ✅ **Phase 3.1 轻量插件系统实现完成**
  - 新增本地工具插件与 Channel 插件加载，插件默认关闭并通过 `enabled` 显式启用
  - `PluginLoader` 限制项目内相对路径，支持非 strict warning 跳过与 strict 启动失败
  - `ToolDispatcher` 增加运行时参数校验，`ChannelRegistry` 提供轻量通道注册边界
  - `LightHermes` 初始化时加载启用的工具与 Channel 插件，默认配置不导入插件
  - 全量测试基线更新为 140/140 通过

**最近更新** (2026-05-11):
- ✅ **v0.3.3 发版完成**
  - 统一源码、打包元数据、CLI 展示和项目文档版本号为 `0.3.3`
  - 完成 Phase 2.7 发版检查：MiniMax smoke test、普通对话回归、配置与文档一致性检查
  - 已创建并推送 `v0.3.3` tag
- ✅ **记忆工具计划完成**
...
- v0.3.3 发版后进入插件系统与工具生态扩展；如需更完整发布前验证，可再补 OpenAI/Anthropic 任一路径真实 API smoke test
