...
class SkillGenerator:
    ...
    def _build_generation_prompt(
        ...
        if pattern_type == "failure":
            return f"""基于以下失败的对话轨迹,生成一个可复用的 Markdown 失败报告。

任务类型: {task_type}
轨迹数量: {trajectory_count}
工具使用序列: {tool_sequences}

**重要**: 请生成 Markdown 格式的失败报告(type: failure_report),而不是正向技能(type: skill)或 Python 插件(type: plugin)。
失败报告用于记录不要重复的做法,帮助后续识别风险和选择替代方案。

请生成一个 Markdown 格式的失败报告文件,包含:
1. 报告名称 (简短的英文标识符)
2. 报告描述 (一句话说明失败模式)
3. 失败模式
4. 不要这样做
...
格式要求:
- 使用 YAML frontmatter,设置 type: failure_report
- frontmatter 包含 name、description、task_type、source: evolution
- 内容部分用清晰的小节
- 保持简洁,避免过度细节
- 不要生成 Python 代码
- 不要把失败轨迹包装成正向执行步骤
...
只返回失败报告文件内容,不要有其他解释。"""
        ...
        return f"""基于以下成功的对话轨迹,生成一个可复用的 Markdown 技能。
...
class SkillValidator:
    ...
    def validate_skill(self, skill: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if skill_type in ("skill", "failure_report"):
            return {"valid": True, "reason": "Markdown 文档无需验证"}
