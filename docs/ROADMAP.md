...
- **记忆智能运行时已具备**：两轮搜索、来源展开、claim 写回、缺席状态、建议改写和停答点强制搜已落地；开发集 A/B 已跑，默认仍关闭。
- **真实评测改变了优先级**：旧静态基线（gpt-5.4-mini）Hit@5 59.0% / QA 50.0%。口径对齐后的 grok-4.6 开发集 A/B：static QA 37.5%，agentic QA 42.5%（+5pp），强制搜 4/40，平均 2.48 次调用。这些数字用于定位问题，不作为产品优化目标或唯一发布门槛。
...
5. ✅ 开发集 static / agentic A/B（证据预算对齐后）。冻结清单见 `docs/FREEZE_LOCK.md`，结果见 `logs/locomo_ab_dev.json`。⬜ holdout 一次。开发集不得再拧参。
...
- [x] 固定数据划分、抽样清单和随机种子，保证 static / agentic A/B 可复现（开发集：`docs/FREEZE_LOCK.md`，seed=42；holdout 划分仍未建）
...
- **基线稳定**: 当前测试基线为 226/226；每个功能完成后跑 `pytest tests/`
