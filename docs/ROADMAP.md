...
- [ ] 定义通用证据状态：`claims`、`evidence`、`conflicts`、`coverage`、`confidence`
- [ ] 根据证据覆盖不足、检索不确定性、来源冲突、可验证性不足和检索增益触发主动搜索
- [ ] 让首次模型调用直接选择“回答”或“调用记忆工具”，不额外引入独立 Planner
- [ ] 查询改写来自未解决 claim 和新发现的 cue anchors，不读取 benchmark 类别或题目模板
- [ ] 合并 TF-IDF、embedding 和多轮候选，去重后统一重排
- [ ] 建立通用 evidence ledger，显式记录每个 claim 的支持来源、冲突、缺口和置信度
- [ ] 按 `source` 读取完整记忆，并通过可选类型关系展开邻居；session 仅作为一种来源适配器
- [ ] 最多两轮主动搜索；达到证据充分、连续无新增证据或预算上限时停止
...
- **基线稳定**: 当前测试基线为 151/151；每个功能完成后跑 `pytest tests/`
...
- [ ] 多轨 static / agentic A/B 达到 3.4 的泛化、质量与成本门槛
