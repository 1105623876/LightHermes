...
1. 完成本地改动的最小化收敛（文档计划同步 + 必要提交）
2. Phase 2 转向记忆蒸馏：删除记忆图谱方向，优先实现分级记忆的压缩、合并、蒸馏和容量治理
3. 借鉴 Hermes 的记忆生命周期钩子，在压缩前、回合结束和会话结束时沉淀工作/情景/语义记忆
4. 借鉴 nanobot 的轻量 agent/tool/skill/channel 架构，先收敛边界，不直接整体集成
5. 在 `memory.py` 补充归档、提升、蒸馏相关边界测试，尤其是 index 同步移除和幂等性
6. 若准备发版，增加一轮真实 API smoke test（OpenAI/Anthropic 任一）
